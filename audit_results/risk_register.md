...
| **RD-03** | Degradation | Nudge konnte Parse-Fehler in geänderten/baseline-relevanten Dateien ohne maschinenlesbare Diagnostik ausgeben, wodurch Commit-Sicherheit überschätzt werden konnte | RPN 192 🟡 | STRIDE T-TB5-02, Issue #47 | Maintainer | Nudge Parse-Visibility | 100% Parse-Fehler im Response sichtbar | Parse-Diagnostik-Felder (`parse_failure_count`, `parse_failed_files`, `parse_failure_treatment`) + Blocking-Hardrule in `safe_to_commit` | **Mitigiert** (Issue #47) | Q2 2026 |
...
| **RD-04** | Degradation | Vendor-/Environment-Dateien wurden als Projektinput verarbeitet und verzerrten Self-Analyse-Laufzeit + Finding-Dichte | RPN 150 🟡 | FMEA DG-06, FTA E8 | Maintainer | Self-Analyse Budget-Einhaltung | ≤ 30s im CI-Budget-Test | Discovery-Default-Excludes für `site-packages`/`.pixi`; Budget-Regressionstest beibehalten | **Mitigiert** (2026-04-02) | Q2 2026 |
